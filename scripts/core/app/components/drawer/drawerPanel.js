...
export class DrawerPanel extends VcsComponent {
    ...
    _expandPanel(index) {
        ...
        const element = ui.VCS.element[0];
        ...
        if (expanded != index) {
            ui.VCS._lastExpandedDrawer = index;
            this.element.querySelector(`.collapsible-panel[data-index="${index}"]`).style.maxHeight = `${scaleHeight}px`;
            el.classList.add("active");
        } else {
            ui.VCS._lastExpandedDrawer = null;
        }
        document.body.classList.toggle("vcs-show-ability-menu", expanded != index);
    }
    ...
    async _renderInner() {
        ...
        this._expandPanel(ui.VCS._lastExpandedDrawer);
        return this.element;
    }
}