...
export class VcsComponent {
    ...
    get actor() {
        return ui.VCS._actor;
    }

    get token() {
        return ui.VCS._token;
    }

    get template() {
        const isAncestorComponent = Object.getPrototypeOf(this.constructor) === VcsComponent;
    ...
    get tooltipCls() {
        return CONFIG.VCS.CORE.Tooltip;
    }
    ...
    async activateTooltipListeners() {
        if (!game.settings.get("vs-combat-suite", "showTooltips")) return;
    ...
    async _onTooltipMouseEnter(event, locked = false) {
        ...
        if (request !== this._tooltipRequest || !this.element.matches(":hover")) return;
        const tooltipData = await this.getTooltipData();
        if (!tooltipData || request !== this._tooltipRequest || !this.element.matches(":hover")) return;
    ...
    async render() {
        ...
        Hooks.callAll(`render${parentClass.name}VcsComponent`, this, this.element, this.actor);
        Hooks.callAll(`render${this.constructor.name}VcsComponent`, this, this.element, this.actor);
        return this.element;
    }
