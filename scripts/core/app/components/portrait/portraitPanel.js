...
import { VcsComponent } from "../component.js";
import {Effect} from "./effect.js";

export class PortraitPanel extends VcsComponent {
    constructor(...args) {
        super(...args);
    ...
    async getData() {
        const data = {
            ...
            canRollDeathSave: this.canRollDeathSave,
            successes: this.successes,
            failures: this.failures,
            playerDetailsBottom: game.settings.get("vs-combat-suite", "playerDetailsBottom"),
        };
        return data;
    }
    ...
    async activateListeners(html) {
        ...
        if (this.configurationTemplate) html.querySelector("#vcs-actor-config")?.addEventListener("click", this._onConfigure.bind(this));
    ...
    async _onDeathSave(event) {
        console.error("VCS: PortraitPanel._onDeathSave not implemented");
    }
    ...
    async _getButtons() {
        return [
                ...
                onClick: async (e) => {
                    if(this.token.combatant?.initiative !== undefined) {
                        const response = await foundry.applications.api.DialogV2.confirm({
                            window: {
                                title: game.i18n.localize(`vs-combat-suite.confirmRerollInitiativeTitle`),
                            },
                            content: game.i18n.localize(`vs-combat-suite.confirmRerollInitiativeContent`),
                            defaultYes: false,
                        });
                        if (!response) return;
                    } 
            ...
            },
            {
                ...
                label: "Minimize",
                onClick: (e) => ui.VCS.toggleMinimize(),
            },
        ];
    }
    ...
    async _renderInner(data) {
        ...
        if (!this.isDead && !this.isDying) {
            ...
            if (this.isDead) {
                ...
                deathBtn.disabled = !this.canRollDeathSave;
            }
        }
        ...
        for (const effect of effectElements) {
            effect.element.addEventListener("contextmenu", async (event) => {
                ...
                const response = await foundry.applications.api.DialogV2.confirm({
                    window: {
                        title: game.i18n.localize(`vs-combat-suite.deleteEffectTitle`),
                    },
                    content: game.i18n.localize(`vs-combat-suite.deleteEffectContent`) + game.i18n.localize(effect?.label ?? statusEffect?.name ?? "") + "?",
                    defaultYes: false,
                });
                if (!response) return;
                if (!effectToDelete) {
    ...
    async _onConfigure(event) {
        ...
        new (getVcsPortraitConfig(template))(this.actor).render({force: true});
    }
}

function getVcsPortraitConfig(template) {
    return class VcsPortraitConfig extends HandlebarsApplication {
        constructor(actor) {
            super();
            this.actor = actor;
        }
