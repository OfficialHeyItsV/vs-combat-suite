...
export class ItemButton extends VcsComponent{
  constructor ({item, isWeaponSet=false, isPrimary=false, inActionPanel=undefined}) {
    ...
    if (this.isWeaponSet) this.hookId = Hooks.on("vcs-onSetChangeUpdateItem", this._onSetChange.bind(this));
    ui.VCS.addItemButtons(this);
  }
  ...
  get useTargetPicker() {
    return game.settings.get("vs-combat-suite", "rangepicker");
  }

  get useRangeFinder() {
    return game.Levels3DPreview?._active && game.settings.get("vs-combat-suite", "rangefinder");
  }
  ...
  async _onPreLeftClick(event) {
    // Start each weapon/spell use with this user's targets only cleared.
    // Activity buttons expose their owning item through the item getter too.
    if (this.requiresTargetSelection || ["weapon", "spell"].includes(this.item?.type)) {
      for (const target of Array.from(game.user.targets)) {
        target.setTarget(false, { releaseOthers: false });
      }
    }
    if ((this.requiresTargetSelection || this.useTargetPicker) && this.targets > 0) {
      isTargetPicker = true;
      const picker = new TargetPicker({token: this.token, targets: this.targets, ranges: this.ranges});
      let result;
      try { result = await picker.promise; }
      finally { isTargetPicker = false; }
      if(!result) return;
    }
    return this._onLeftClick(event);
  }
...
export async function showRangeFinder(range, object){
  ...
  const sett = game.settings.get("vs-combat-suite", "rangefinder")
