...
export class AccordionPanel extends VcsComponent {
    ...
    toggle(toggle, noTransition = false) {
        ...
        if (toggle) ui.VCS.collapseAllPanels();
    ...
    saveState() {
        ...
        ui.VCS.setPanelState(state, this);
    }

    restoreState() {
        if (!this.id) return;
        const state = ui.VCS.getPanelState(this);
        if (!state) return;
    ...
    async toggleDefaults() {
        let totalActionBarWidth = ui.VCS.actionBarWidth;
        for (const panel of this._subPanels) {
