...
export class WeaponSets extends VcsComponent {
    get classes() {
        return ["weapon-sets"];
    }
    ...
    async activateListeners(html) {
        ...
        const activeSet = this.actor.getFlag("vs-combat-suite", "activeWeaponSet") || "1";
        this.element.querySelectorAll(".weapon-set").forEach((element) => {
            element.classList.toggle("active", element.dataset.set === activeSet);
        });

        this.onSetChange(await this.getSetData(), { updateEquipment: false });
    }

    async getSetData() {
        const sets = await this._getSets();
        const active = this.actor.getFlag("vs-combat-suite", "activeWeaponSet") || "1";
        return { sets, active };
    }

    async _getSets() {
        const sets = foundry.utils.mergeObject(await this.getDefaultSets(), foundry.utils.deepClone(this.actor.getFlag("vs-combat-suite", "weaponSets") || {}));
    ...
    async _onDrop(event) {
        try {
            ...
            const sets = this.actor.getFlag("vs-combat-suite", "weaponSets") || {};
            sets[set] = sets[set] || {};
            sets[set][slot] = data.uuid;

            await this.actor.setFlag("vs-combat-suite", "weaponSets", sets);
            await this.render();
        } catch (error) {}
    }
    ...
    async _onClick(event) {
        ...
        await this.actor.setFlag("vs-combat-suite", "activeWeaponSet", set);
    ...
    async onSetChange({ sets, active }, { updateEquipment = true } = {}) {
        Hooks.callAll("vcs-onSetChangeUpdateItem", { sets, active });
        Hooks.callAll("vcs-onSetChangeComplete", { sets, active });
        if (updateEquipment) await this._onSetChange({ sets, active });
    }
