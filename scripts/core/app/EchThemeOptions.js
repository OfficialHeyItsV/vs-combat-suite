...
export class EchThemeOptions extends HandlebarsApplication {
    ...
    static get PARTS() {
        return {
            content: {
                template: "modules/vs-combat-suite/templates/theme-options.hbs",
    ...
    createTheme() {
        new foundry.applications.api.DialogV2({
            window: { title: "Export V's Combat Suite Theme" },
            content: `
                <form id="vcsExportTheme">
                    <div class="form-group">
                        <label>${game.i18n.localize("vs-combat-suite.themeOptions.exportThemeName")}</label>
                        <input type="text" name="vcsExportThemeName"/>
                    </div>
                </form>`,
            buttons: [
                {
                    ...
                    callback: (event, button, dialog)  => {
                        const themeName = dialog.element.querySelector('input[name="vcsExportThemeName"]').value.slugify({strict: true});
                        const theme = new File([new Blob([JSON.stringify(this.getThemeColors())], { type: "application/json" })], `${themeName}.json`);
                        foundry.applications.apps.FilePicker.implementation.uploadPersistent("vs-combat-suite", "themes", theme).then((response) => {
    ...
    getThemeColors() {
        ...
        this.element.querySelectorAll('#vcsThemeOptions .window-content vcs-alpha-color-picker').forEach((input) => {
    ...
    async buildDropdown(selectedTheme) {
        if (typeof selectedTheme == "undefined") game.settings.get("vs-combat-suite", "echThemeData").theme;

        const echThemeOptions = this.element;
        const selectTheme = echThemeOptions.querySelector('select[name="theme"]');
        selectTheme.innerHTML = "";

        const coreThemes = (await foundry.applications.apps.FilePicker.implementation.browse("user", `modules/vs-combat-suite/scripts/themes`, { extensions: [".json"] })).files;
        const customThemes = (await foundry.applications.apps.FilePicker.implementation.browse("user", `modules/vs-combat-suite/storage/themes`, { extensions: [".json"] })).files;
    ...
    _prepareContext(options) {
        ...
        let themeOptions = game.settings.get("vs-combat-suite", "echThemeData");
    ...
    _onRender(context, options) {
        ...
        this.buildDropdown(game.settings.get("vs-combat-suite", "echThemeData").theme);
        ...
        html
            ...
            .addEventListener("change", (event) => {
                ...
                if (selectedTheme != "custom") {
                    ui.VCS.getThemeJson(selectedTheme).then((theme) => {
                        updateColors(theme);
                    });
                } else {
                    updateColors(game.settings.get("vs-combat-suite", "echThemeData").colors);
                }
                this.setThemeVisibility(selectedTheme);
                this.setPosition({height: "auto"});
            });
    ...
    _onChangeForm(formConfig, event) {
        ...
        ui.VCS.setColorSettings(foundry.utils.expandObject(formData));
    }
    ...
    static async #onSubmit() {
        ...
        await game.settings.set("vs-combat-suite", "echThemeData", foundry.utils.expandObject(formData));
    }

    setThemeVisibility(selectedTheme) {
        this.element.querySelector(".vcsThemeOptionsContainer").classList.toggle("hidden", selectedTheme != "custom");
