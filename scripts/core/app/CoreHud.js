import { VcsComponent } from "./components/component.js";
...
export class CoreHud extends HandlebarsApplication {
  constructor() {
    ...
    Hooks.callAll(`vcsInit`, CoreHud);
    Hooks.on(
      "vcs-onSetChangeComplete",
    ...
    Hooks.on("canvasReady", () => this.openPreferredActor());
    Hooks.on("updateUser", (user, changes) => {
      if (user.id === game.user.id && "character" in changes) this.openPreferredActor();
    });
    ...
    document.addEventListener(
      "wheel",
      (event) =>
        ui.VCS._tooltip && ui.VCS._tooltip.setScrollDelta(event.deltaY)
    );
  }
  ...
  //     id: "vs-combat-suite",
  //     template: "modules/vs-combat-suite/templates/coreHUD.hbs",
  //     popOut: false,
  ...
  static get PARTS() {
    return {
      content: {
        template: `${TEMPLATE_PATH}core-hud.hbs`,
        classes: [],
        scrollable: [],
      },
    };
  ...
  static get DEFAULT_OPTIONS() {
    return mergeClone(super.DEFAULT_OPTIONS, {
      classes: ["vcs-combat-hud", `fade-out-inactive-${game.settings.get(
        "vs-combat-suite",
        "fadeOutInactive"
      )}`],
      window: {
  ...
  _onUpdateItem(item, changes = {}) {
    ...
    if (item.type === "weapon" && ("system.equipped" in changes || changes.system?.equipped !== undefined)) {
      this.components.weaponSets?.render();
    }
    ...
    if (!controlled && !canvas.tokens.controlled.length) {
      setTimeout(() => {
        if (!canvas.tokens.controlled.length) {
          if (this.autoOpen) this.openPreferredActor();
          else this.bind(null);
        }
      }, 100);
    }
    if (!controlled) return;
    if (token.actor?.isOwner && (game.activeTool === "select" || this.enabled || this.autoOpen)) {
      this.bind(token);
    }
  }

  get autoOpen() {
    return !game.user.isGM || game.settings.get("vs-combat-suite", "alwaysOn");
  }

  openPreferredActor() {
    if (!this.autoOpen) return;
    const selected = globalThis.canvas?.ready
      ? canvas.tokens?.controlled?.find(token => token.actor?.isOwner)
      : null;
    const character = game.user.character?.isOwner ? game.user.character : null;
    const target = selected ?? character;
    if (target !== this._target) return this.bind(target);
  }
  ...
  _onRender(context, options) {
    this.toggleMinimize(document.body.classList.contains("minimize-vcs-hud"));
  ...
  async bind(target) {
    ...
    if (!this._actor || !this._actor.isOwner) {
      ...
      this._token = null;
      this._actor = null;
      console.error("VCS: No actor found");
      return this.close();
    }
    ...
    if (supportedActorTypes.length && !supportedActorTypes.includes(actorType)) {
      ...
      if (!suppressWarnings)
        ui.notifications.error(
          localize("vs-combat-suite.err.unsupportedActorType").replace("%t",actorType)
        );
      else
        console.error(
          localize("vs-combat-suite.err.unsupportedActorType").replace("%t",actorType)
  ...
  updateSceneControlButton() {
    const btn = document.querySelector(`.control-tool[data-tool="vcsToggle"]`);
    if (!btn) return;
    try {
      btn.classList.toggle("active", !!this._target);
      const tool = ui.controls?.controls?.tokens?.tools?.vcsToggle;
      if (tool) tool.active = !!this._target;
    } catch (e) { }
  }
  ...
  toggleUiElements(toggle) {
    ...
    [hotbar, fps, players]
      ...
      .forEach((element) => {
        element.classList.toggle("vcs-hidden", toggle);
      });
  }
  ...
  toggleMinimize(forceState) {
    ...
    const isMinimizeEchHud = body.classList.contains("minimize-vcs-hud");

    const newState = forceState ?? !isMinimizeEchHud;

    body.classList.toggle("minimize-vcs-hud", newState);
  ...
  setPosition(preventTransition = true) {
    ...
    const scale = !game.settings.get("vs-combat-suite", "autoScale")
      ? game.settings.get("vs-combat-suite", "scale")
      : window.innerHeight / 2000; //(1 / (echHUDWidth / windowWidth)) * game.settings.get("vs-combat-suite", "scale");
    const appliedScale = scale > 1 ? 1 : scale;

    const position = {
      bottom: isMinimizeHud
        ? "0px"
        : `${game.settings.get("vs-combat-suite", "botPos")}px`,
      left: `${game.settings.get("vs-combat-suite", "leftPos")}px`,
      transform: isMinimizeHud
        ? `scale(${appliedScale}) translateY(100%)`
        : `scale(${appliedScale})`,
      width: `calc(100vw * ${scale < 1 ? 1 + parseFloat(1 - scale) * 1 : 1})`,
    };

    html.style.setProperty("--vcs-hud-scale", appliedScale);
  ...
  performModuleCheck() {
    ...
    const systemModuleElement = `<strong>${game.system.id.toUpperCase()}</strong>`;
    ui.notifications.error(
      localize("vs-combat-suite.err.moduleNotActive").replace(
        "%m",
        systemModuleElement
      ),
      { permanent: true }
  ...
  interceptNextDialog(element) {
    ...
    const hookId = Hooks.once("renderDialog", (dialog, html) => {
      ...
      if (game.settings.get("vs-combat-suite", "dialogTheme"))
        html.classList.add("vcs-highjack-window");
  ...
  static setControlHooks() {
    Hooks.on("drawToken", (token) => {
      // Foundry activates its own mouse manager immediately after drawToken,
      // clearing token listeners. Attach after that activation completes.
      queueMicrotask(() => {
        if (token.destroyed) return;
        if (token._vcsSelectClickListener) token.off("pointertap", token._vcsSelectClickListener);
        token._vcsSelectClickListener = (event) => {
          if (event.button !== 0 || event.shiftKey || game.activeTool !== "select" || !token.actor?.isOwner) return;
          const hud = ui.VCS;
          if (hud && (hud._target !== token || !hud.rendered)) hud.bind(token);
        };
        token.on("pointertap", token._vcsSelectClickListener);
      });
    });
    Hooks.on("getSceneControlButtons", (controls, b, c) => {
      controls.tokens.tools.vcsToggle = {
        active: ui.VCS?._target,
        icon: "fa-duotone fa-swords",
        name: "vcsToggle",
        title: game.i18n.localize("vs-combat-suite.controls.toggle.title"),
        onChange: function (event, active) {
          ui.VCS.toggle(active);
        },
        toggle: true,
      };
    });
  ...
  async setColorSettings(preview) {
    document.documentElement.style.setProperty(
      "--vcs-fadeout-deleay",
      game.settings.get("vs-combat-suite", "fadeoutDelay") + "s"
    );
    document.documentElement.style.setProperty(
      "--vcs-fadeout-opacity",
      game.settings.get("vs-combat-suite", "fadeoutOpacity")
    );
    ...
    let theme = preview ?? game.settings.get("vs-combat-suite", "echThemeData");
    ...
    const noBlur = game.settings.get("vs-combat-suite", "noBlur");

    if (noBlur)
      document.documentElement.style.setProperty("--vcs-blur-amount", "");
  }

  async getThemeJson(themeName) {
    let file;
    file = await fetch(
      `modules/vs-combat-suite/scripts/themes/${themeName}.json`
    );
    if (!file.ok)
      file = await fetch(
        `modules/vs-combat-suite/storage/themes/${themeName}.json`
      );
    if (!file.ok) {
      ui.notifications.error("VCS: Theme not found");
      file = await fetch(`modules/vs-combat-suite/scripts/themes/vcs.json`);
    }
