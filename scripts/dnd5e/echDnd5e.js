...
import { getRitualSpells, ritualUseConfig } from "./rituals.js";
import { useMidiHudAttack, registerMidiHudAutomation } from "./hud-damage.js";
...
export function initConfig() {
    registerMidiHudAutomation();

    Hooks.on("updateItem", (item) => {
        if(item.parent !== ui.VCS._actor || !ui.VCS.rendered) return;
        ui.VCS.components.portrait.refresh();
        if (item.type === "spell") ui.VCS.refresh();
    })

    Hooks.on("vcsInit", (CoreHUD) => {
        if (game.system.id !== "dnd5e") return;
        registerItems();
        setExplodeItemActivities();
        const VCS = CoreHUD.VCS;

        class DND5eTooltip extends VCS.CORE.Tooltip {
            get classes() {
        ...
        const getMidiFlag = (actionType) => {
            if (!isMIDI || !ui.VCS._actor) return null;
            const flag = ui.VCS._actor.getFlag("midi-qol", "actions") ?? {};
        ...
        Hooks.callAll("vcs-combat-hud.dnd5e.initConfig", { actionTypes, itemTypes, ECHItems });
        ...
        async function getTooltipDetails(item, type) {
            ...
            if (type == "skill") {
                ...
                description = game.i18n.localize(`vs-combat-suite-dnd5e.skills.${item}.tooltip`);
            } else if (type == "save") {
                title = CONFIG.DND5E.abilities[item].label;
                description = game.i18n.localize(`vs-combat-suite-dnd5e.abilities.${item}.tooltip`);
            } else {
            ...
            if (target || range) {
                details = [
                    {
                        label: "vs-combat-suite-dnd5e.tooltip.target.name",
                        value: target,
                    },
                    {
                        label: "vs-combat-suite-dnd5e.tooltip.range.name",
                        value: range,
                    },
                ];
            }
            if (item?.labels?.toHit) {
                details.push({
                    label: "vs-combat-suite-dnd5e.tooltip.toHit.name",
            ...
            if (item?.labels?.damages?.length) {
                ...
                details.push({
                    label: "vs-combat-suite-dnd5e.tooltip.damage.name",
                    value: dmgString,
                });
            }
            ...
            for (let i = 0; i < barItemsLength; i++) {
                ...
                if (isCondensedButton) {
                    if (currentSplitButtonItemButton) {
                        ...
                        condensedItems.push(new VCS.MAIN.BUTTONS.SplitButton(currentSplitButtonItemButton, button));
        ...
        class DND5ePortraitPanel extends VCS.PORTRAIT.PortraitPanel {
            constructor(...args) {
                super(...args);
            }
            ...
            get isDead() {
                return this.isDying && !this.actor.system.attributes?.death;
            }
            ...
            get canRollDeathSave() {
                return this.isDying && !!this.actor.isOwner
                    && !!this.actor.system.attributes?.death
                    && typeof this.actor.rollDeathSave === "function";
            }
            ...
            get configurationTemplate() {
                return "modules/vs-combat-suite/templates/dnd5e/vcs-actor-config.hbs";
            }

            async _onDeathSave(event) {
                event.preventDefault();
                event.stopPropagation();
                if (!this.canRollDeathSave || this._rollingDeathSave) return;
                const actor = this.actor;
                this._rollingDeathSave = true;
                try {
                    return await actor.rollDeathSave({ event, legacy: false });
                } finally {
                    this._rollingDeathSave = false;
                }
            }
                ...
                return [
                    ...
                    [
                        ...
                        },
                        {
                            ...
                            color: "var(--vcs-movement-baseMovement-background)",
                        },
                    ],
                    [
                        {
        ...
        class DND5eDrawerButton extends VCS.DRAWER.DrawerButton {
        ...
        class DND5eDrawerPanel extends VCS.DRAWER.DrawerPanel {
            constructor(...args) {
                super(...args);
            }
            ...
            get categories() {
                ...
                return [
                    ...
                    },
                    {
                        ...
                        captions: [
                            {
                                label: game.i18n.localize("vs-combat-suite-dnd5e.hud.tools.name"),
                            },
                            {
                                label: "",
                            },
            ...
            get title() {
                return `${game.i18n.localize("vs-combat-suite-dnd5e.hud.saves.name")} / ${game.i18n.localize("vs-combat-suite-dnd5e.hud.skills.name")} / ${game.i18n.localize("vs-combat-suite-dnd5e.hud.tools.name")}`;
            }
        }

        class DND5eActionActionPanel extends VCS.MAIN.ActionPanel {
            constructor(...args) {
                super(...args);
            }
            ...
            async _getButtons() {
                ...
                const ritualItems = getRitualSpells(this.actor.items);
                const ritualButton = !ritualItems.length ? [] : [new DND5eButtonPanelButton({ type: "ritual", items: ritualItems, color: 0 })].filter((button) => button.hasContents);
                ...
                if (showSpecialActions) {
                    buttons.push(...[new DND5eItemButton({ item: null, isWeaponSet: true, isPrimary: true }), new VCS.MAIN.BUTTONS.SplitButton(new DND5eSpecialActionButton(specialActions[0]), new DND5eSpecialActionButton(specialActions[1])), ...spellButton, ...ritualButton, new DND5eButtonPanelButton({ type: "feat", items: featItems, color: 0 }), new VCS.MAIN.BUTTONS.SplitButton(new DND5eSpecialActionButton(specialActions[2]), new DND5eSpecialActionButton(specialActions[3])), new VCS.MAIN.BUTTONS.SplitButton(new DND5eSpecialActionButton(specialActions[4]), new DND5eSpecialActionButton(specialActions[5])), new DND5eButtonPanelButton({ type: "consumable", items: consumableItems, color: 0 })]);
                } else {
                    buttons.push(...[new DND5eItemButton({ item: null, isWeaponSet: true, isPrimary: true }), ...spellButton, ...ritualButton, new DND5eButtonPanelButton({ type: "feat", items: featItems, color: 0 }), new DND5eButtonPanelButton({ type: "consumable", items: consumableItems, color: 0 })]);
                }
        ...
        class DND5eBonusActionPanel extends VCS.MAIN.ActionPanel {
            constructor(...args) {
                super(...args);
            }
        ...
        class DND5eReactionActionPanel extends VCS.MAIN.ActionPanel {
            constructor(...args) {
                super(...args);
            }
        ...
        class DND5eFreeActionPanel extends VCS.MAIN.ActionPanel {
            constructor(...args) {
                super(...args);
            }
        ...
        class DND5eLegActionPanel extends VCS.MAIN.ActionPanel {
            constructor(...args) {
                super(...args);
            }
        ...
        class DND5eLairActionPanel extends VCS.MAIN.ActionPanel {
            constructor(...args) {
                super(...args);
            }
        ...
        class DND5eMythicActionPanel extends VCS.MAIN.ActionPanel {
            constructor(...args) {
                super(...args);
            }
        ...
        class DND5eItemButton extends VCS.MAIN.BUTTONS.ItemButton {
            constructor(...args) {
                super(...args);
                this._isRitual = args[0]?.ritual === true;
            }
            ...
            get requiresTargetSelection() {
                const activities = this.isActivity ? [this.activity] : Array.from(this.item?.system?.activities ?? []);
                return activities.some(a => (a?.type === "attack"
                    && (this.item?.type === "weapon" || a.attack?.type?.classification === "unarmed"))
                    || (this.item?.type === "spell" && !a.target?.template?.type && !a.target?.template?.units
                        && ["creature", "ally", "enemy"].includes(a.target?.affects?.type)));
            }
            ...
            get targets() {
                ...
                return this.requiresTargetSelection ? 1 : null;
            }
            ...
            async getTooltipData() {
                ...
                tooltipData.propertiesLabel = "vs-combat-suite-dnd5e.tooltip.properties.name";
                return tooltipData;
            }

            async _onLeftClick(event) {
                // ui.VCS.interceptNextDialog(event.currentTarget);
                // const used = await this.activity.use({event, legacy: false}, {event});
                const usage = this._isRitual ? ritualUseConfig(event) : { event, legacy: false };
                const midiSpell = this.item?.type === "spell" && game.modules?.get("midi-qol")?.active;
                const isDirectAttack = activity => activity?.type === "attack"
                    && (this.item?.type === "weapon" || activity.attack?.type?.classification === "unarmed");
                // Use native unarmed classification, independent of the item's name or document type.
                // Resolve the native multi-activity chooser before configuring its selected action.
                const chooser = globalThis.dnd5e?.applications?.activity?.ActivityChoiceDialog;
                if (!this.isActivity && chooser
                    && (midiSpell || (!this._isRitual && Array.from(this.item?.system?.activities ?? []).some(isDirectAttack)))) {
                    const activities = Array.from(this.item.system.activities ?? []).filter(a => a.canUse);
                    if (activities.length > 1) {
                        const selected = event?.shiftKey ? activities[0] : await chooser.create(this.item);
                        if (!selected) return;
                        const result = midiSpell || isDirectAttack(selected)
                            ? await DND5eItemButton.useHudAttack(selected, usage, event)
                            : await selected.use(usage, { event });
                        if (result && !this._isRitual) DND5eItemButton.consumeActionEconomy(selected);
                        return result;
                    }
                }

                const dialog = { event };
                const directAttack = (midiSpell || (!this._isRitual && isDirectAttack(this.activity)))
                    && (this.isActivity || this.item?.system?.activities?.size === 1);
                if (!this.isActivity && !directAttack) return this.item.use(usage, dialog);
                const used = directAttack
                    ? await DND5eItemButton.useHudAttack(this.activity, usage, event)
                    : await this.activity.use(usage, dialog);
                if (used) {
                    if (!this._isRitual) DND5eItemButton.consumeActionEconomy(this.activity);
                    ...
                    if (useOtherItem) {
                        ...
                        ui.VCS.updateItemButtons(allConnectedItems);
                    }
                    this.render(true)
                }
            }

            static async useHudAttack(activity, usage, event) {
                const tokens = Array.from(game.user.targets ?? []);
                const pointer = ui.VCS._pointerPosition ?? { x: event?.clientX, y: event?.clientY };
                const width = globalThis.innerWidth ?? 1280;
                const height = globalThis.innerHeight ?? 800;
                const position = {
                    left: Math.max(8, Math.min((pointer.x ?? width / 2) + 16, width - 420)),
                    top: Math.max(8, Math.min((pointer.y ?? height / 2) - 80, height - 480)),
                };
                if (game.modules?.get("midi-qol")?.active) {
                    return useMidiHudAttack(activity, usage, event, position, tokens);
                }

                const targets = tokens.filter(token => token.actor).map(token => ({
                    name: token.name, img: token.actor.img, uuid: token.actor.uuid,
                    ac: token.actor.statuses?.has("coverTotal") ? null : token.actor.system?.attributes?.ac?.value,
                }));
                if (!targets.length || targets.some(target => !Number.isFinite(target.ac))) {
                    ui.notifications.warn("Select a creature with an Armor Class before attacking.");
                    return;
                }
                const restoreTargets = () => {
                    for (const token of tokens) if (!game.user.targets.has(token)) token.setTarget(true, { releaseOthers: false });
                };
                // Own this continuation explicitly; disable the system's automatic one to prevent two rolls.
                const result = await activity.use({ ...usage, subsequentActions: false }, { event, configure: false }, { create: false });
                if (!result?.message) return result;
                // Mirror native chat Attack handling so already-spent resources and scaling are preserved.
                ...
                const item = (consumed || scaling) ? activity.item.clone({
                    "flags.dnd5e": { consumed, scaling },
                }, { keepId: true }) : activity.item;
                const rollActivity = item.system.activities.get(activity.id);
                restoreTargets();
                const rollMessage = { data: {
                    "flags.dnd5e.targets": targets,
                    ...(message.id ? { "flags.dnd5e.originatingMessage": message.id } : {}),
                }};
                const rolls = await rollActivity.rollAttack({ event, target: targets.length === 1 ? targets[0].ac : undefined },
                    { configure: true, options: { position } }, rollMessage);
                const roll = rolls?.[0];
                if (!roll || roll.isFumble) return result;
                const hit = roll.isCritical || targets.some(target => Number(roll.total) >= target.ac);
                if (hit) {
                    restoreTargets();
                    await rollActivity.rollDamage({ event, isCritical: Boolean(roll.isCritical),
                        attackMode: roll.options?.attackMode, ammunition: roll.options?.ammunition },
                        { configure: true, options: { position } }, { data: { "flags.dnd5e.targets": targets } });
                }
                return result;
            }
            ...
            static consumeActionEconomy(activity) {
                ...
                if (actionType === "action") {
                    ui.VCS.components.main[0].isActionUsed = true;
                    ui.VCS.components.main[0].updateActionUse();
                } else if (actionType === "bonus") {
                    ui.VCS.components.main[1].isActionUsed = true;
                    ui.VCS.components.main[1].updateActionUse();
                } else if (actionType === "reaction") {
                    ui.VCS.components.main[2].isActionUsed = true;
                    ui.VCS.components.main[2].updateActionUse();
                } else if (actionType === "free") {
                    ui.VCS.components.main[3].isActionUsed = true;
                    ui.VCS.components.main[3].updateActionUse();
                } else if (actionType === "legendary") {
                    ui.VCS.components.main[4].isActionUsed = true;
                }
            }
            ...
            async render(...args) {
                ...
                if (this.activity) {
                    ...
                    ui.VCS.updateItemButtons(weapons);
                }
            }
        ...
        class DND5eButtonPanelButton extends VCS.MAIN.BUTTONS.ButtonPanelButton {
            ...
            get label() {
                switch (this.type) {
                    case "spell":
                        return "vs-combat-suite-dnd5e.hud.castspell.name";
                    case "ritual":
                        return "vs-combat-suite-dnd5e.hud.rituals.name";
                    case "feat":
                        return "vs-combat-suite-dnd5e.hud.usepower.name";
                    case "consumable":
                        return "vs-combat-suite-dnd5e.hud.useitem.name";
                    case "weapon":
                        return "vs-combat-suite-dnd5e.hud.useitem.name";
                }
            }
            ...
            get icon() {
                switch (this.type) {
                    case "spell":
                    case "ritual":
                        return "modules/vs-combat-suite/icons/spell-book.webp";
                    case "feat":
                        return "modules/vs-combat-suite/icons/mighty-force.webp";
                    case "consumable":
                        return "modules/vs-combat-suite/icons/drink-me.webp";
                    case "weapon":
                        return "modules/vs-combat-suite/icons/drink-me.webp";
                }
            }
            ...
            async _getPanel() {
                if (this.type === "spell") {
                    return new VCS.MAIN.BUTTON_PANELS.ACCORDION.AccordionPanel({ id: this.id, accordionPanelCategories: this._spells.map(({ label, buttons, uses }) => new VCS.MAIN.BUTTON_PANELS.ACCORDION.AccordionPanelCategory({ label, buttons, uses })) });
                } else if (this.type === "ritual") {
                    const accordionPanelCategories = Object.entries(CONFIG.DND5E.spellLevels)
                        .sort(([a], [b]) => Number(a) - Number(b))
                        .map(([level, label]) => ({ label, items: this.items.filter((item) => Number(item.system?.level) === Number(level)) }))
                        .filter(({ items }) => items.length)
                        .map(({ label, items }) => new VCS.MAIN.BUTTON_PANELS.ACCORDION.AccordionPanelCategory({
                            label,
                            buttons: items.map((item) => new DND5eItemButton({ item, ritual: true })),
                        }));
                    return new VCS.MAIN.BUTTON_PANELS.ACCORDION.AccordionPanel({ id: this.id, accordionPanelCategories });
                } else {
                    return new VCS.MAIN.BUTTON_PANELS.ButtonPanel({ id: this.id, buttons: this.items.map((item) => new DND5eItemButton({ item })) });
                }
            }
        }

        class DND5eSpecialActionButton extends VCS.MAIN.BUTTONS.ActionButton {
    ...
    <section class="card-header description collapsible">
        ...
        class DND5eMovementHud extends VCS.MovementHud {
            get movementMax() {
        ...
        class DND5eButtonHud extends VCS.ButtonHud {
            constructor(...args) {
                super(...args);
            }

            get visible() {
                const targetedAvailable = this.actor?.isOwner && typeof globalThis.TargetedSpecialAttacks?.open === "function";
                return !game.combat?.started || targetedAvailable;
            }
            ...
            async _getButtons() {
                ...
                // V's Targeted Attacks is optional. Only expose its action when
                // the API is ready and the bound actor belongs to the user.
                if (targetedAvailable) {
                    buttons.push({
                        label: "vs-combat-suite-dnd5e.hud.targetedAttack.name",
                        icon: "fas fa-crosshairs",
                        onClick: () => {
                            const api = globalThis.TargetedSpecialAttacks;
                            if (this.actor?.isOwner && typeof api?.open === "function") {
                                return api.open(this.actor);
                            }
                        },
                    });
                }
        ...
        class DND5eWeaponSets extends VCS.WeaponSets {
            async getDefaultSets() {
                const sets = await super.getDefaultSets();
                const isTransformed = this.actor.flags?.dnd5e?.isPolymorphed;
                if (this.actor.type !== "npc" && !isTransformed) {
                    const saved = this.actor.getFlag(MODULE_ID, "weaponSets") ?? {};
                    const assigned = new Set(Object.values(saved).flatMap(slots => Object.values(slots)).filter(Boolean));
                    const equipped = this.actor.items.filter(item => item.type === "weapon" && item.system?.equipped && !assigned.has(item.uuid));
                    for (const [set, slots] of Object.entries(sets)) {
                        for (const slot of ["primary", "secondary"]) {
                            if (!saved[set]?.[slot]) slots[slot] = equipped.shift()?.uuid ?? null;
                        }
                    }
                    return sets;
                }
            ...
            async _getSets() {
                const isTransformed = this.actor.flags?.dnd5e?.isPolymorphed;
                const saved = foundry.utils.deepClone(this.actor.getFlag(MODULE_ID, "weaponSets") || {});
                if (this.actor.type !== "npc" && !isTransformed) {
                    for (const slots of Object.values(saved)) {
                        for (const slot of ["primary", "secondary"]) if (!slots[slot]) delete slots[slot];
                    }
                }
                const sets = isTransformed ? await this.getDefaultSets() : foundry.utils.mergeObject(await this.getDefaultSets(), saved);
            ...
            async _onSetChange({ sets, active }) {
                ...
                const configured = this.actor.getFlag(MODULE_ID, "weaponSets")?.[active];
                // Auto-filled slots mirror already-equipped inventory. Merely
                // selecting them must not unequip other automatic slots.
                if (this.actor.type === "character" && !configured?.primary && !configured?.secondary) return;
                const updates = [];
        ...
        if (enableMacroPanel) mainPanels.push(VCS.PREFAB.MacroPanel);
        mainPanels.push(VCS.PREFAB.PassTurnPanel);
...
function registerItems() {
    ECHItems[game.i18n.localize("vs-combat-suite-dnd5e.items.disengage.name")] = {
        name: game.i18n.localize("vs-combat-suite-dnd5e.items.disengage.name"),
        type: "feat",
        img: "modules/vs-combat-suite/icons/journey.webp",
        system: {
            type: {
                value: "",
                subtype: "",
            ...
            description: {
                value: game.i18n.localize("vs-combat-suite-dnd5e.items.disengage.desc"),
                chat: "",
                unidentified: "",
            },
            source: "",
    ...
    ECHItems[game.i18n.localize("vs-combat-suite-dnd5e.items.dodge.name")] = {
        name: game.i18n.localize("vs-combat-suite-dnd5e.items.dodge.name"),
        type: "feat",
        img: "modules/vs-combat-suite/icons/armor-upgrade.webp",
        system: {
            type: {
                value: "",
                subtype: "",
            ...
            description: {
                value: game.i18n.localize("vs-combat-suite-dnd5e.items.dodge.desc"),
                chat: "",
                unidentified: "",
            },
            source: "",
    ...
    ECHItems[game.i18n.localize("vs-combat-suite-dnd5e.items.ready.name")] = {
        name: game.i18n.localize("vs-combat-suite-dnd5e.items.ready.name"),
        type: "feat",
        img: "modules/vs-combat-suite/icons/clockwork.webp",
        system: {
            type: {
                value: "",
                subtype: "",
            ...
            description: {
                value: game.i18n.localize("vs-combat-suite-dnd5e.items.ready.desc"),
                chat: "",
                unidentified: "",
            },
            source: "",
    ...
    ECHItems[game.i18n.localize("vs-combat-suite-dnd5e.items.hide.name")] = {
        name: game.i18n.localize("vs-combat-suite-dnd5e.items.hide.name"),
        type: "feat",
        img: "modules/vs-combat-suite/icons/cloak-dagger.webp",
        system: {
            type: {
                value: "",
                subtype: "",
            ...
            description: {
                value: game.i18n.localize("vs-combat-suite-dnd5e.items.hide.desc"),
                chat: "",
                unidentified: "",
            },
            source: "",
    ...
    ECHItems[game.i18n.localize("vs-combat-suite-dnd5e.items.dash.name")] = {
        name: game.i18n.localize("vs-combat-suite-dnd5e.items.dash.name"),
        type: "feat",
        img: "modules/vs-combat-suite/icons/walking-boot.webp",
        system: {
            type: {
                value: "",
                subtype: "",
            ...
            description: {
                value: game.i18n.localize("vs-combat-suite-dnd5e.items.dash.desc"),
                chat: "",
                unidentified: "",
            },
            source: "",
    ...
    ECHItems[game.i18n.localize("vs-combat-suite-dnd5e.items.shove.name")] = {
        name: game.i18n.localize("vs-combat-suite-dnd5e.items.shove.name"),
        type: "feat",
        img: "modules/vs-combat-suite/icons/shield-bash.webp",
        system: {
            type: {
                value: "",
                subtype: "",
            ...
            description: {
                value: game.i18n.localize("vs-combat-suite-dnd5e.items.shove.desc"),
                chat: "",
                unidentified: "",
            },
            source: "",
